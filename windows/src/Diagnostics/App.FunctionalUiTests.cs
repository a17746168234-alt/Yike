...
public partial class App {
	...
	private async Task VerifyFunctionalUiAsync() {
		...
		try {
			...
			foreach(string page in new[] { "appearance","deepl","speech","shortcuts","permissions","ocr","history","about" }) {
				...
				UiCheck(!UiDescendants(first.Content as DependencyObject).OfType<TextBlock>().Any(t=>t.Text=="账号与安全"), "removed account UI reappeared");
			...
			File.WriteAllText(report,"PASS: all eight settings pages navigate in one window; actual assembly version displayed; voice gender/speed/local-online and history/OCR preferences applied; idle playback controls disabled; live component refresh completed; popup close clears active window; header button descendants excluded from dragging. Preview tests do not write user preferences.\n" + RuntimeStatus.Microphone() + "\n" + RuntimeStatus.Recognizers() + "\n" + await OcrService.InstalledLanguages());
