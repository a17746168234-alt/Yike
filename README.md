...
当前安装包为 **1.6（Build 64），Apple Silicon / arm64**。Apple 翻译需要 **macOS 15 或以上**，每台 Mac 首次使用需联网确认并下载对应语言包；安装包不包含语言包或 DeepL 密钥。本应用的 Apple 翻译不支持 Windows。个人 DeepL 需要用户自行设置 API Free 密钥；公共体验可在设置的“账号与安全”注册领取额度，服务开启状态以页面提示为准。
...
- 设置新增“账号与安全”：注册、登录、退出、修改密码及额度查询；会话独立保存在系统钥匙串。
- 每个账号一次赠送 50,000 字符，所有用户共享 1,000,000 字符总预算，按实际使用先到先用，用完即止，不按月重置。注册不会预留公共池额度。
- 未登录仍可使用 Apple 翻译和个人 DeepL 密钥；公共服务密钥只存服务器，不包含在安装包、客户端或源码中。
- 当前账号服务已部署，**公共翻译暂未开启**，等待配置独立的公共服务密钥。现有个人密钥不会被读取或用于公共服务。
- 公共翻译经过 Yike 服务器发送到 DeepL；账号页说明数据处理和暂不支持邮箱找回。实际可用额度也受上游供应商限制。
- 服务端部署、密钥配置和 Windows 接口说明见 [server/README.md](server/README.md)。
