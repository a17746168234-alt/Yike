...
macOS V2.3 Build 75 安装包通过 `https://n5v1b.cn/yike-download/Yike-macOS-arm64-v2.3-build75.dmg` 提供同域下载。Nginx 仅将这个固定路径映射到 `/opt/yike-trial/public/Yike-macOS-arm64-v2.3-build75.dmg`；上线前和 GitHub Release 资产核对 SHA-256，再更新 `update-macos.json`。GitHub Release 仍保留安装包下载入口。
