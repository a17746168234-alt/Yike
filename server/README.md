...
macOS v2.5 Build 77 安装包通过 `https://n5v1b.cn/yike-download/Yike-macOS-arm64-v2.5-build77.dmg` 提供同域下载。Nginx 将版本固定路径映射到 `/opt/yike-trial/public/Yike-macOS-arm64-v2.5-build77.dmg`。每次发布 macOS 版本都必须先上传并核对服务器 DMG 的 SHA-256，再更新服务器 `update-macos.json` 和对应 Nginx 路径；GitHub Release 同时保留安装包下载入口。
